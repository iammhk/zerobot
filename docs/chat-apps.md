...
Connect zerobot to your favorite chat platform. Want to build your own? See the [Channel Plugin Guide](./channel-plugin-guide.md).

| Channel | What you need |
|---------|---------------|
| **Telegram** | Bot token from @BotFather |
| **Discord** | Bot token + Message Content intent |
| **WhatsApp** | QR code scan (`zerobot channels login whatsapp`) |
| **WeChat (Weixin)** | QR code scan (`zerobot channels login weixin`) |
...
- You need:
  - `userId` (example: `@zerobot:matrix.org`)
  - `password`
...
> After upgrading zerobot, rebuild the local bridge with:
> `rm -rf ~/.zerobot/bridge && zerobot channels login whatsapp`
...
- **Permissions**:
  ...
  - **Streaming replies** (default in zerobot): add **`cardkit:card:write`** (often labeled **Create and update cards** in the Feishu developer console). Required for CardKit entities and streamed assistant text. Older apps may not have it yet — open **Permission management**, enable the scope, then **publish** a new app version if the console requires it.
...
> `allowFrom`: Add your open_id (find it in zerobot logs when you message the bot). Use `["*"]` to allow all users.
...
> - `allowFrom`: Add your openid (find it in zerobot logs when you message the bot). Use `["*"]` for public access.
...
> - `allowFrom`: Add the sender ID you see in zerobot logs for your WeChat account. Use `["*"]` to allow all users.
> - `token`: Optional. If omitted, log in interactively and zerobot will save the token for you.
> - `routeTag`: Optional. When your upstream Weixin deployment requires request routing, zerobot will send it as the `SKRouteTag` header.
> - `stateDir`: Optional. Defaults to zerobot's runtime directory for Weixin state.
> - `pollTimeout`: Optional long-poll timeout in seconds.

**3. Login**

```bash
zerobot channels login weixin
```

Use `--force` to re-authenticate and ignore any saved token:

```bash
zerobot channels login weixin --force
```
...
```bash
pip install zerobot-ai[msteams]
```
...
> - `mentionOnlyResponse` controls what zerobot receives when a user sends only a bot mention (`<at>zerobot</at>`). Set to `""` to ignore mention-only messages.
