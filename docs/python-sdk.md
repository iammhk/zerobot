...
Use zerobot as a library — no CLI, no gateway, just Python.
...
`zerobot.from_config()` reuses your normal `~/.zerobot/config.json`, so the SDK follows the same provider, model, tools, and workspace defaults as the CLI unless you override them.
...
bot = zerobot.from_config(
    config_path="~/.zerobot/config.json",
...
Hooks let you inspect tool calls, streaming, and iteration state without modifying zerobot internals:

```python
from zerobot.agent import AgentHook, AgentHookContext
...
### `zerobot.from_config(config_path=None, *, workspace=None)`

Create a `zerobot` instance from a config file.

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.zerobot/config.json`. |
...
```python
from zerobot.agent import AgentHook
...
async def main() -> None:
    bot = zerobot.from_config(workspace="/my/project")
