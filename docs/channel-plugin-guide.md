...
Build a custom zerobot channel in three steps: subclass, package, install.

> **Note:** We recommend developing channel plugins against a source checkout of zerobot (`pip install -e .`) rather than a PyPI release, so you always have access to the latest base-channel features and APIs.

## How It Works

zerobot discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `zerobot gateway` starts, it scans:

1. Built-in channels in `zerobot/channels/`
2. External packages registered under the `zerobot.channels` entry point group
...
```text
zerobot-channel-webhook/
├── zerobot_channel_webhook/
...
```python
# zerobot_channel_webhook/__init__.py
from zerobot_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# zerobot_channel_webhook/channel.py
import asyncio
...
from zerobot.config.schema import Base
...
# pyproject.toml
[project]
name = "zerobot-channel-webhook"
version = "0.1.0"
dependencies = ["zerobot-ai", "aiohttp"]

[project.entry-points."zerobot.channels"]
webhook = "zerobot_channel_webhook:WebhookChannel"
...
packages = ["zerobot_channel_webhook"]
```
...
```bash
pip install -e .
zerobot plugins list      # verify "Webhook" shows as "plugin"
zerobot onboard           # auto-adds default config for detected plugins
```

Edit `~/.zerobot/config.json`:
...
```bash
zerobot channels login <channel_name>
zerobot channels login <channel_name> --force  # re-authenticate
```
...
| `default_config()` (classmethod) | Returns default config dict for `zerobot onboard`. Override to declare your fields. |
...
Built-in channels use Pydantic config models (subclassing `Base` from `zerobot.config.schema`). Plugin channels **must do the same**.

### Pattern

1. Define a Pydantic model inheriting from `zerobot.config.schema.Base`:
...
Override `default_config()` so `zerobot onboard` auto-populates `config.json`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `zerobot-channel-{name}` | `zerobot-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `zerobot_channel_{name}` | `zerobot_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/zerobot-channel-webhook
cd zerobot-channel-webhook
pip install -e .
zerobot plugins list    # should show "Webhook" as "plugin"
zerobot gateway         # test end-to-end
```

## Verify

```bash
$ zerobot plugins list
