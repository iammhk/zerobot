...
from zerobot.channels.matrix import _build_matrix_text_content

import zerobot.channels.matrix as matrix_module
from zerobot.bus.events import OutboundMessage
from zerobot.bus.queue import MessageBus
from zerobot.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from zerobot.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("zerobot.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "zerobot.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("zerobot.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "zerobot.channels.matrix.asyncio.create_task", _fake_create_task
    )
