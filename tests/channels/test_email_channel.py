...
from zerobot.channels.email import EmailConfig
...
def test_fetch_new_messages_skips_self_sent_email_and_marks_seen(monkeypatch) -> None:
    raw = _make_raw_email(from_addr="zerobot <bot@example.com>", subject="Loop test")
...
def test_fetch_new_messages_retries_once_when_imap_connection_goes_stale(monkeypatch) -> None:
    ...
    monkeypatch.setattr("zerobot.channels.email.imaplib.IMAP4_SSL", _factory)
...
def test_fetch_new_messages_keeps_messages_collected_before_stale_retry(monkeypatch) -> None:
    ...
    monkeypatch.setattr("zerobot.channels.email.imaplib.IMAP4_SSL", lambda _h, _p: FlakyIMAP())
...
def test_fetch_new_messages_skips_missing_mailbox(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "zerobot.channels.email.imaplib.IMAP4_SSL",
...
def test_extract_attachments_saves_pdf(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("zerobot.channels.email.get_media_dir", lambda ch: tmp_path)
