...
from zerobot.command.builtin import cmd_new, register_builtin_commands
from zerobot.command.router import CommandContext, CommandRouter
from zerobot.config.schema import AgentDefaults, Config
...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("zerobot.agent.loop.SessionManager"), \
         patch("zerobot.agent.loop.SubagentManager") as MockSubMgr, \
         patch("zerobot.agent.loop.Dream"):
...
class TestUnifiedSessionConfig:
    ...
    def test_onboard_generated_config_contains_unified_session(self, tmp_path: Path):
        """save_config() writes 'unifiedSession' into config.json (simulates zerobot onboard)."""
        from zerobot.config.loader import save_config
...
class TestStopCommandWithUnifiedSession:
    ...
    async def test_stop_command_finds_task_in_unified_mode(self, tmp_path: Path):
        ...
        from zerobot.agent.loop import UNIFIED_SESSION_KEY
        from zerobot.command.builtin import cmd_stop
