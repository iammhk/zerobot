...
- Publish to Zerobot bus via BaseChannel._handle_message()
...
from zerobot.security.network import validate_url_target

try:
    from zerobot.config.paths import get_media_dir
...
def _make_bot_class(channel: QQChannel) -> type[botpy.Client]:
    ...
    class _Bot(botpy.Client):
        def __init__(self):
            # Disable botpy's file log — Zerobot uses loguru; default "botpy.log" fails on read-only fs
...
class QQConfig(Base):
    ...
    # Optional: directory to save inbound attachments. If empty, use Zerobot get_media_dir("qq").
    media_dir: str = ""
