...
from zerobot.config.paths import get_media_dir, get_runtime_subdir
...
class WeixinChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                logger.error("WeChat login failed. Run 'Zerobot channels login weixin' to authenticate.")
                self._running = False
                return
    ...
    async def _send_text(
        ...
        client_id = f"Zerobot-{uuid.uuid4().hex[:12]}"
