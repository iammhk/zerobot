...
from zerobot.utils.restart import consume_restart_notice_from_env, format_restart_completed_message
...
def _default_webui_dist() -> Path | None:
    ...
    try:
        import zerobot.web as web_pkg  # type: ignore[import-not-found]
    except ImportError:
        return None
...
class ChannelManager:
    ...
    def _init_channels(self) -> None:
        """Initialize enabled channels only (lazy loading)."""
        from zerobot.channels.registry import (
            discover_channel_names,
            discover_plugins_lazy,
            load_channel_class,
        )
        ...
        builtin_names = discover_channel_names()
        plugins = discover_plugins_lazy()

        # Iterate over all possible channel names
        all_names = set(builtin_names) | set(plugins.keys())
        ...
        for name in all_names:
            ...
            try:
                # ONLY NOW we load the class and its heavy dependencies
