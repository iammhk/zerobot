...
from zerobot import __logo__, __version__
...
from zerobot.cli.stream import StreamRenderer, ThinkingSpinner
from zerobot.config.paths import get_workspace_path, is_default_workspace
from zerobot.config.schema import Config
from zerobot.utils.helpers import sync_workspace_templates
...
def _init_prompt_session() -> None:
    ...
    from zerobot.config.paths import get_cli_history_path
...
def onboard(
    ...
    if wizard:
        from zerobot.cli.onboard import run_onboard
        ...
        try:
            ...
            console.print("[yellow]Please run 'Zerobot onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
    ...
    agent_cmd = 'Zerobot agent -m "Hello!"'
    gateway_cmd = "Zerobot gateway"
    if config:
...
def _onboard_plugins(config_path: Path) -> None:
    ...
    from zerobot.channels.registry import discover_all
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from zerobot.config.loader import load_config, resolve_config_env_vars, set_config_path
...
def _warn_deprecated_config_keys(config_path: Path | None) -> None:
    ...
    from zerobot.config.loader import get_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from zerobot.config.paths import get_cron_dir
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show Zerobot runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    try:
        ...
        console.print("[red]aiohttp is required. Install with: pip install 'Zerobot-ai[api]'[/red]")
        raise typer.Exit(1)

    from loguru import logger
    from zerobot.agent.loop import AgentLoop
    from zerobot.api.server import create_app
    ...
    agent_loop = AgentLoop(
        ...
        hardware_type=runtime_config.agents.defaults.hardware_type,
    )
...
def _run_gateway(
    ...
    *,
    ...
    console.print(f"{__logo__} Starting Zerobot gateway version {__version__} on port {port}...")
    ...
    # Expose it on the agent so slash commands (e.g. /voice-on) can reach it.
...
def channels_status(
    ...
    """Show channel status."""
    from zerobot.channels.registry import discover_all
    from zerobot.config.loader import load_config, set_config_path
...
def _get_bridge_dir() -> Path:
    ...
    from zerobot.config.paths import get_bridge_install_dir
...
def plugins_list():
    ...
    from zerobot.channels.registry import discover_all, discover_channel_names
...
def status():
    """Show Zerobot status."""
    from zerobot.config.loader import get_config_path, load_config
...
def _login_github_copilot() -> None:
    try:
        from zerobot.providers.github_copilot_provider import login_github_copilot
