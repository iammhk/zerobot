...
def turn_step(s_ch, k_ch, dir):
    ...
    # L1: Back (+), R1: Forward (+), L2: Forward (+), R2: Back (+)
    offset = 35 * dir
    set_angle(s_ch, mid_s + offset)
    time.sleep(0.12)
...
try:
    ...
    for _ in range(10): # 10 cycles
        ...
        turn_step(L1, L3, 1) # FL moves back
        turn_step(R1, R3, 1) # FR moves forward
        turn_step(L2, L4, 1) # HL moves forward
        turn_step(R2, R4, 1) # HR moves back
